...
function main() {
    // axios
    //     .get(`https://raw.githubusercontent.com/penggguna/QuranJSON/master/quran.json`)
    //     .then((response) => {
            ...
            <div class="col">
                <div class="card shadow-sm">
                    <svg class="bd-placeholder-img card-img-top" width="100%" height="225" xmlns="http://www.w3.org/2000/svg" role="img" preserveAspectRatio="xMidYMid slice" focusable="false"><rect width="100%" height="100%" fill="#1356c2"/>
                        <text x="50%" y="50%" fill="#eceeef" dy=".3em">Q.S ${data.name}</text>
                        <text x="50%" y="65%" fill="#eceeef" dy=".3em">${data.name_translations.ar} - ${data.name_translations.id}</text>
                    </svg>

                    <div class="card-body">
                        <p class="card-text">Number of Surah : ${data.number_of_surah}</p>
                        <p class="card-text">Number of Ayah : ${data.number_of_ayah}</p>
        ...
        axios
            ...
            .then((response) => {
                ...
                <div class="card shadow-sm mt-lg-4">
                    <svg class="bd-placeholder-img card-img-top" width="100%" height="225" xmlns="http://www.w3.org/2000/svg" role="img" preserveAspectRatio="xMidYMid slice" focusable="false"><rect width="100%" height="100%" fill="#1356c2"/>
                        <text x="50%" y="50%" fill="#eceeef" dy=".3em">Q.S ${response.data.name}</text>
                        <text x="50%" y="65%" fill="#eceeef" dy=".3em">${response.data.name_translations.ar} - ${response.data.name_translations.id}</text>
                    </svg>

                    <div class="card-body">
                        <p class="card-text">Number of Surah : ${response.data.number_of_surah}</p>
                        <p class="card-text">Number of Ayah : ${response.data.number_of_ayah}</p>
                        <p class="card-text">Type of Surah : ${response.data.type}</p>
                        <br>
